...
## Acceptance criteria
- [ ] `data/entities.json` exists with a schema for at least Chests and NPCs.
- [ ] `src/core/registry.py` can load the JSON and instantiate the correct `Interactable` subclasses.
- [ ] Unit tests verify that an entity created from the registry has the correct initial state and components.
